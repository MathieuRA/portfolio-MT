...
class Carousel extends React.Component {
  constructor(props) {
    ...
    if (props.loop === false && props.auto) {
      console.warn(
        '[re-carousel] Auto-slide only works in loop mode.'
      )
    }
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const frames = [].concat(
      nextProps.frames || nextProps.children || []
    )
    const nextState = { frames }
    if (
      frames.length &&
      frames.length !== prevState.frames.length
    ) {
  ...
  decideEndPosition() {
    ...
    switch (axis) {
      case 'x':
        if (loop === false) {
          if (currentFrameIndex === 0 && deltaX > 0)
            return 'origin'
          if (
      ...
      case 'y':
        if (loop === false) {
          if (currentFrameIndex === 0 && deltaY > 0)
            return 'origin'
          if (
            currentFrameIndex === frames.length - 1 &&
            deltaY < 0
  ...
  autoSlide(rel) {
    ...
    switch (rel) {
      case 'prev':
        this.transitFramesTowards(
          this.props.axis === 'x' ? 'right' : 'down'
        )
        break
      case 'next':
      default:
        this.transitFramesTowards(
          this.props.axis === 'x' ? 'left' : 'up'
        )
    }
  ...
  setFrame(index, ms = undefined) {
    ...
    const diff = Math.abs(
      index - this.state.currentFrameIndex
    )
  ...
  prepareSiblingFrames(
    ...
    const siblings = override
      ? this.getSiblingFramesByOverride(index)
      : this.getSiblingFrames()
  ...
  transitFramesTowards(direction) {
    ...
    let newCurrentFrameIndexId = this.state
      .currentFrameIndex
    switch (direction) {
      case 'up':
        translateXY(
    ...
    onTransitionEnd &&
      setTimeout(
        () => onTransitionEnd(this.getSiblingFrames()),
        duration
      )

    this.setState({
      currentFrameIndex: newCurrentFrameIndexId,
    })
  }
  ...
  render() {
    ...
    const wrapperStyle = objectAssign(
      styles.wrapper,
      this.props.style
    )
        ...
        >
          {frames.map((frame, i) => {
            const frameStyle = objectAssign(
              {
                zIndex:
                  99 - Math.abs(currentFrameIndex - i),
              },
              styles.frame
            )
            return (
